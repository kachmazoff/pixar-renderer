        ...
        const int &CAM_Y_OFFSET = 0)
    {
        for (size_t f = 0; f < faces.size(); ++f)
        {
            ...
            for (size_t p = 0; p < pointNums.size(); ++p)
            {
                ...
                z.push_back(points[pointNums[p]].z() * ZOOM);
            }
            ...
            // uint curr_color = (int(r_rand * ang) << 16 | int(g_rand * ang) << 8 | int(b_rand * ang));
            // 172,116,52
            // uint curr_color = (int(172 * ang) << 16 | int(116 * ang) << 8 | int(52 * ang));
            ...
            for (size_t px_x = x_min; px_x < x_max; ++px_x)
            {
                for (size_t px_y = y_min; px_y < y_max; ++px_y)
                {
                    ...
                    if (is_inside(coords)) {
                        ...
                        image.set_pixel(px_x, px_y, curr_z * cum.z(), curr_color);
                    }
                }
            }
        }
